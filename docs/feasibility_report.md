...
## 1. 摘要
LiteOS作为面向IoT领域的轻量级实时操作系统，采用C语言开发，存在缓冲区溢出、释放后使用、数据竞争等内存安全问题，难以满足IoT设备高安全需求。本项目计划以**已完成Rust改写的`los_memory.c`为基础**，继续用Rust重构剩余11个内核核心模块（含任务管理、调度器、IPC组件等），同时引入**双层IronClaw部署架构**作为智能交互与安全执行的核心链路：**PC端完整IronClaw**负责接收用户自然语言输入（如"给我的小宠物吃点饭，然后让他睡一觉"），调用大语言模型将其改写为结构化指令；**开发板端IronClaw-Lite**负责对下发指令进行参数合法性校验与能力权限（capability-based）分析，再通过C/Rust FFI驱动LiteOS-M内核执行。本文聚焦可行性分析，阐述待改写模块的Rust适配性、分阶段实现路径、双层IronClaw接口设计，论证在保持轻量实时特性的同时，从语言层面提升内核内存安全与交互可靠性的可行性。
...
本项目采用**双层IronClaw部署架构**，将自然语言理解与嵌入式端安全执行严格分离，形成完整的"自然语言输入 → 结构化指令 → 权限校验 → 内核执行"链路。
...
（如"给我的小宠物吃点饭，然后让他睡一觉"）
   ↓
【第一层】PC本地完整IronClaw（自然语言理解层）
   - 调用LLM（nearai/OpenAI/Anthropic/Ollama等）理解用户意图
   - 将自然语言改写为结构化指令序列（如 FEED 50 → SLEEP）
   - 通过WiFi将结构化指令下发至开发板
   ↓（WiFi传输）
【第二层】开发板IronClaw-Lite（参数权限分析层）
   - 参数合法性校验（范围、类型、边界）
   - 能力权限（capability-based）分析
   - 拒绝非法指令，防止越权操作
   ↓
C/Rust FFI → LiteOS-M内核执行
   ↓
执行结果通过UART/WiFi上报至PC端IronClaw
```

**第一层：PC本地完整IronClaw（自然语言理解层）**

PC端运行完整的IronClaw实例，具备以下能力：
- **自然语言理解**：接收用户口语化输入，调用后端LLM解析意图，识别动作序列（喂食、睡觉、玩耍等）及参数（喂食量、时长等）；
- **指令改写**：将模糊的自然语言转化为开发板可执行的结构化文本指令（如`FEED 50`、`SLEEP`、`PLAY 10`），并按逻辑顺序排列；
- **结果汇总**：接收开发板上报的执行结果，生成可读的状态报告或性能分析；
- **完整IronClaw能力**：WASM沙箱、持久化记忆、Routines调度、多渠道接入（REPL/HTTP/WebSocket）等。

**第二层：开发板IronClaw-Lite（参数权限分析层）**

开发板端运行轻量化的IronClaw-Lite，不具备LLM推理能力，专注于安全执行：
- **参数合法性校验**：验证指令格式、参数范围（如喂食量∈[1,100]）、参数数量是否符合规范；
- **能力权限分析**：基于capability-based权限模型，检查当前指令是否在允许的能力白名单内（`CAP_READ`/`CAP_WRITE`/`CAP_STRESS`等）；
- **拒绝非法操作**：对越权或格式错误的指令返回明确错误码，不进入内核执行路径；
- **轻量化设计**：无GC、无LLM依赖，编译为静态库链接进LiteOS-M固件，适配352KB SRAM的资源约束。

本项目引入**IronClaw-Lite轻量化中间层（Agent）**，作为开发板侧用户层（应用程序）与Rust重构内核层的统一对接入口，屏蔽语言差异、权限隔离、调用规范，实现**用户→内核、内核→用户双向安全调用**，是项目核心交互组件。
#### 3.5.1 IronClaw（Agent）植入部位
双层架构中，两层IronClaw的植入位置各有侧重：

**PC端完整IronClaw（第一层）**：运行于开发者PC，不植入嵌入式固件。其核心植入点是用户交互入口与WiFi发送模块——接收自然语言输入后，调用LLM改写为结构化指令，再通过TCP/UDP发送至开发板。

**开发板IronClaw-Lite（第二层）**：植入位置严格锚定LiteOS原有系统调用链路，无侵入式修改原有框架，具体植入层级与部位如下：
1. **用户态-内核态边界层（核心植入点）**
挂载于LiteOS原生SVC（Supervisor Call）异常处理入口，替代原有C实现的系统调用分发逻辑：
- 物理位置：`los_syscall.c`（原有系统调用分发文件）的`SVC_Handler`中断处理函数内，将系统调用分发逻辑接管至IronClaw-Lite层；
- 作用：所有从PC端下发的结构化指令，先进入IronClaw-Lite进行参数合法性校验、能力权限检查，再转发至Rust重构内核模块，避免非法调用直接触达内核。
2. **内核层回调转发层（辅助植入点）**
...
- 作用：内核层产生的主动通知/回调请求，先经由IronClaw-Lite完成跨语言类型转换、用户态权限映射，再推送至用户层，保证回调的安全性与兼容性。
3. **编译构建链路（部署植入）**
在LiteOS构建系统（Makefile/CMake）中，将IronClaw-Lite编译单元（C/Rust混合实现）链接至内核镜像的`os_adapter`段，与原有C内核代码段隔离，保证内存布局兼容性，且支持独立编译调试。
...
项目核心目标是验证 Rust 部分重写 LiteOS-M 后的任务、IPC、内存管理和双层 IronClaw 安全接口链路，而非蓝牙协议栈或多开发板移植能力。因此采用 **WiFi + UART 保底调试** 的主线方案。

**双层 IronClaw 在本案例中的分工：**

- **PC端完整IronClaw**：运行于开发者笔记本，接收用户自然语言输入（如"给我的小宠物吃点饭，然后让他睡一觉"），调用LLM将其解析为结构化指令序列（`FEED 50` → `SLEEP`），通过WiFi TCP/UDP发送至Hi3861V100；
- **开发板IronClaw-Lite**：运行于Hi3861V100，接收PC端下发的结构化指令，执行参数合法性校验与capability权限分析，校验通过后通过C/Rust FFI驱动LiteOS-M内核执行，并将结果上报回PC端。
...
`PC端IronClaw（自然语言→结构化指令）-> WiFi -> Hi3861V100 IronClaw-Lite（参数/权限校验）-> C/Rust FFI -> LiteOS-M内核执行 -> UART/WiFi上报结果`

EcoPet 是系统验证场景，不是外设堆料场景。其验证对象与体现如下：

| 验证对象 | EcoPet 中的体现 |
|---|---|
| PC端IronClaw自然语言理解 | 用户说"给宠物喂饭"，PC端LLM解析为`FEED 50` |
| WiFi指令下发 | PC端将结构化指令通过TCP/UDP发送至Hi3861V100 |
| LiteOS-M 启动 | Hi3861V100 上电后进入 LiteOS-M 主任务 |
| C/Rust FFI | C 任务调用 Rust 导出函数进行命令校验与状态更新 |
| IronClaw-Lite | 对下发指令进行能力检查、参数检查、错误码返回 |
| 任务管理 | 创建接收任务、状态任务、上报任务 |
| IPC | 通过 LiteOS-M Queue 将命令从接收任务传给状态任务 |
| 内存管理 | 通过受控命令进行小规模 alloc/free 压测 |
| WiFi 输入 | PC端IronClaw通过 WiFi 发送结构化文本命令 |
...
- **双层IronClaw智能交互架构**：PC端完整IronClaw负责自然语言理解（LLM改写指令），开发板端IronClaw-Lite负责参数权限校验，两层协作形成"自然语言→结构化指令→权限校验→内核执行"完整链路，将LLM推理与嵌入式安全执行解耦；
- **IronClaw-Lite统一交互层**：标准化开发板侧用户-内核双向调用接口，兼容原有生态的同时提升交互可靠性；
- **混合内核架构**：保留C语言内核框架，Rust重构核心逻辑，兼顾实时性与安全性；
- **Hi3861V100真机闭环验证**：以单块开发板为载体，将PC端自然语言输入、WiFi指令下发、IronClaw-Lite权限校验、LiteOS-M启动、C/Rust FFI、Queue IPC、Rust状态机串联为完整上板演示链路，在资源受限（352KB SRAM、2MB Flash）的真实硬件环境中验证系统可行性。
